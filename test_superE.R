...
#   for testing of SuperEnhancer model fitting package (Dukler, et al, 2017)
#
# C Heiser, 2018
...
if(!interactive()){
  ...
  parser$add_argument('-ab', '--activitybounds',  default='c(-200, 200)',
                      help='Activity bounds to pass to superEnhancerDataObject() function. Default [-200, 200].')
  ...
  parser$add_argument('-lf', '--linkfunctions', default="c('additive', 'exponential', 'logistic')",
                      help='Link functions to test as R vector. Default Additive, Exponential, and Logistic.')
  parser$add_argument('-ef', '--errorfunctions', default="c('gaussian', 'lognormal')",
                      help='Error functions to test as R vector. Default Gaussian and Log-Normal.')
  ...
  # retrieve the rest of the parameters for test function
  ...
  link.functions <- eval(parse(text=args$linkfunctions))
  error.models <- eval(parse(text=args$errorfunctions))
